...
def fig_fila(res):
    """Gera gráfico da evolução do tamanho da fila de espera"""
...
def fig_ocupacao(res):
    """Gera gráfico da evolução da ocupação dos médicos"""
...
def fig_fila_vs_lambda(base_params, pessoas=None, lam_min=10, lam_max=30, step=2, reps=10):
    """Gera gráfico da relação entre tamanho médio da fila e taxa de chegada (λ)"""
    lambdas = []
    medias = []
...
def show_plot(title, fig_func, *args, **kwargs):
    """Mostra gráfico numa janela GUI"""
    if sg is None:
        return
