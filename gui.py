# gui.py
"""
Interface gráfica da aplicação usando FreeSimpleGUI
"""
import os
...
def parse_int(s, default):
    """Converte string para int com valor default em caso de erro"""
    v = default
    try:
        v = int(str(s).strip())
    except Exception:
...
def parse_float(s, default):
    """Converte string para float com valor default em caso de erro"""
    v = default
    try:
        v = float(str(s).strip())
    except Exception:
...
def format_res(res):
    """Formata resultados da simulação para exibição"""
    par = res["parametros"]
