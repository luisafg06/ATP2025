...
def simula(
    ...
    while queue_empty(queueEventos) == False:
        ...
        if tipo == CHEGADA:
            id_medico_livre = None
            i = 0
            while i < len(medicos) and id_medico_livre is None:
                if not m_ocupado(medicos[i]):
                    id_medico_livre = i
                i = i + 1

            if id_medico_livre is not None:
                medico = medicos[id_medico_livre]
                medico = mOcupa(medico)
                medico = mInicioConsulta(medico, tempo_atual)
                medico = mDoenteCorrente(medico, doente)
                medicos[id_medico_livre] = medico
