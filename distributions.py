# distributions.py
"""
Funções para geração de tempos aleatórios com diferentes distribuições
"""
import numpy as np
from config import TEMPO_MEDIO_CONSULTA, DISTRIBUICAO_TEMPO_CONSULTA, STD_NORMAL


def gera_intervalo_tempo_chegada(taxa_chegada_por_minuto):
    """Gera intervalo entre chegadas usando distribuição exponencial"""
    rate = float(taxa_chegada_por_minuto)
    inter = float("inf")
    if rate > 0:
        inter = float(np.random.exponential(1.0 / rate))
    return inter


def gera_tempo_consulta(media=TEMPO_MEDIO_CONSULTA, 
                        distribuicao=DISTRIBUICAO_TEMPO_CONSULTA, 
                        std=STD_NORMAL):
    """Gera tempo de consulta de acordo com a distribuição especificada"""
    dist = str(distribuicao).strip().lower()
    mean = float(media)

    t = 0.0
    if dist == "exponential":
        t = float(np.random.exponential(mean))
    elif dist == "uniform":
        t = float(np.random.uniform(0.5 * mean, 1.5 * mean))
    elif dist == "normal":
        t = float(np.random.normal(mean, float(std)))
        if t < 0:
            t = 0.0
    else:
        t = float(np.random.exponential(mean))
    return t
